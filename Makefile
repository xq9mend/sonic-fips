...
$(addprefix $(TARGET_PATH)/, $(MAIN_TARGETS)) : $(TARGET_PATH)/% : $$(addprefix $(TARGET_PATH)/,$$($$*_DEPENDS))
	...
	# Apply series of patches if exist (exit loudly on any quilt failure)
	if [ -f $($*_SRC_PATH).patch/series ]; then \
	  pushd $($*_SRC_PATH) && QUILT_PATCHES=../$(notdir $($*_SRC_PATH)).patch quilt push -a; \
	...
	if [ -n "$($*_PATCH_EXT)" ]; then \
	  pushd $($*_SRC_PATH); QUILT_PATCHES=$($*_PATCH_EXT) quilt push -a; \
	  QUILT_RC=$$?; if [ $$QUILT_RC -ne 0 ]; then echo "ERROR: quilt push -a failed (rc=$$QUILT_RC) for $($*_PATCH_EXT)" >&2; exit $$QUILT_RC; fi; \
	  mv .pc .pc2; popd; fi
